...
ob_start();
	...
	if (isset($_POST["btn_pay"])) {
		// Validate form data (implement proper validation here)
	  
		$sellerId = $_GET["did"]; // Assuming 'did' in URL contains seller ID
		$plan = $_GET["plan"]; // Assuming 'plan' in URL contains chosen plan (3, 6, or 12)
	  
		// Calculate subscription end date based on plan
		$today = new DateTime();
		$interval = DateInterval::createFromDateString("{$plan} months");
		$endDate = $today->add($interval)->format('Y-m-d');
	  
		// Update seller subscription information (assuming a database connection is established)
		$sql = "UPDATE tbl_seller SET seller_amount='".$_GET['price']."', seller_subscribe='$endDate' WHERE seller_id='$sellerId'";
		$conn->query($sql);

		$insQr = "insert into tbl_subscribe(subscribe_amount,seller_id,subscribe_date)values('".$_GET['price']."','$sellerId',curdate())";
	  
		$conn->query($insQr);
		if ($conn->affected_rows > 0) {
			echo "<script>alert('Subscription Completed please Wait till you are  verified !');</script>";

		  echo "<script>window.location='Login.php';</script>"; // Redirect to login page on success
		} else {
		  echo "<script>alert('Subscription update failed!');</script>";
		}
	  }
	  ?>
